  ...
  <form method="post" action="{{route('photo.store')}}" enctype="multipart/form-data">
    ...
    <div class="mb-3">
      ...
      <select class="form-control" id="post_id" name="post_id">
        ...
        <option {{old('post_id') == $post->id ? 'selected' : null}} value="{{$post->id}}">
          {{$post->title}}</option>
        @endforeach
      </select>
    </div>
    <div class="mb-3">
      ...
      <img class="mt-2 d-none" src="#" id="photo_preview" style="width: 120px; max-width: 100%" />
    </div>
    <div class="mb-3">
...
<script>
  ...
  photoInput.onchange = (e) => {
    ...
    if (file) {
      ...
      photoPreviewImg.setAttribute('class', 'mt-2 d-inline-block');
    }
  };
</script>
@endsection